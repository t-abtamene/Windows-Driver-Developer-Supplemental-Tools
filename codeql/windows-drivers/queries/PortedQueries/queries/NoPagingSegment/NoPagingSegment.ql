...
from Function f
where not isInPagedCodeSection(f) and f instanceof PagedFunc and not f.getAnAttribute().getName() = ["code_seg", "alloc_text"]
select f, ""
...
// from MacroInvocation mi, TypedefType td
// where mi.getMacroName() = "PAGED_CODE_SEG"
// select mi, mi.
